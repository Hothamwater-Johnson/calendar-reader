...
## 2026-06-16 — Calendar picker (biggest fix): only count calendars you choose
**Decision:** Added a **Calendars** tab listing every calendar with an Include? (Yes/No) column, built by `buildCalendarsSheet_` (run from "Set up sheet" and a new "Choose calendars" menu item). New calendars default to Include=Yes only when `calendar.isOwnedByMe()` is true; subscribed calendars default No. Existing choices are preserved on refresh. `getBusyIntervals_` and `scanCalendar` now skip non-included calendars via `getIncludedCalendarNames_` / `isCalendarIncluded_` (fallback = owned-only when the tab doesn't exist yet).
**Reasoning:** The first real diagnostic showed the user's wife was subscribed to ~10 coworkers' full calendars plus shared/holiday calendars, and the tool counted ALL of them as her busy time — so she looked booked whenever any colleague was. This dwarfed the booking-page issue. Letting her pick calendars (defaulting to ones she owns) is the highest-impact correctness fix. Matching by calendar name keeps the tab human-readable for a non-technical user; `isOwnedByMe()` cleanly separates her calendars from subscribed ones. Booking-page detection still pending a focused re-scan (her calendars only, Calendar service enabled).
